...
setwd("~/Library/CloudStorage/OneDrive-MassGeneralBrigham/van Galen, Peter's files - Sternum_BM/Sternum_BM_Flow/Sternum_BM_Flow_Github/AnalysisVaishali")
rm(list = ls())
...
volcano_plot <- function(data, cell_list, short_labels, conditions, condition_titles, dir){
  ...
  for(i in 1:length(conditions)){
    ...
    for(row in 1:nrow(comparison)){
      if(!is.na(comparison$`adjusted`[row]) & !is.na(comparison$LogFC[row])){
        if(comparison$`adjusted`[row] > 1.301 & comparison$LogFC[row] > 0.3){
          comparison$color[row] <- "Increased"
        } else if(comparison$`adjusted`[row] > 1.301 & comparison$LogFC[row] < -0.3){
    ...
    ggsave(paste0(dir, "/", title, "_volcano_v1.pdf"), p, device = "pdf", width = 12, height = 10)
    message("Saved: ", title, "_volcano_v1.pdf")
  }
}
